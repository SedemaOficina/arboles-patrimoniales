...
/* Metadatos por EJEMPLAR.
   ...
   buscador que esta no debe indexarse por sí misma: las trece fichas se
   consolidaban en el inicio y ninguna aparecía en resultados. Al compartir
   cualquier ficha en redes salía el título y la foto de la portada.
   El ejemplar que se ensambla ya está aquí, así que se compone con sus datos.
   El resto de las fichas se pintan del lado del cliente sobre este mismo
   documento; para ellas el canonical apunta a su propio fragmento. */
const _reg = JSON.parse(fs.readFileSync('datos/registro.json','utf8'));
const _ej = (_reg.ejemplares||_reg).find(e=>e.slug===slug) || (_reg.ejemplares||_reg)[0] || {};
const _esc = (t) => String(t==null?'':t).replace(/[&<>"]/g, c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c]));
const _nombre = _ej.nombreAsignado || 'Ficha del ejemplar';
const _titulo = `${_nombre} · Árboles patrimoniales de la Ciudad de México`;
const _partes = [
  _ej.especie ? `${_ej.especie}${_ej.nombreComun?` (${_ej.nombreComun})`:''}.` : '',
  _ej.alcaldia ? `Alcaldía ${_ej.alcaldia}.` : '',
...
<title>${_esc(_titulo)}</title>
  ...
  <meta property="og:title" content="${_esc(_titulo)}">
  <meta property="og:description" content="${_esc(_desc)}">
  <meta property="og:url" content="${_urlFicha}">
  <meta property="og:image" content="${SITIO.urlTarjeta()}">
  <meta property="og:image:width" content="1200">
  <meta property="og:image:height" content="630">
  <meta property="og:image:alt" content="${_esc(_nombre)}, árbol patrimonial de la Ciudad de México">
  <meta name="twitter:card" content="summary_large_image">
  <meta name="twitter:title" content="${_esc(_titulo)}">
...
<!-- Leaflet lo descarga leaflet-diferido.js cuando el recuadro del mapa se
     acerca a la pantalla; en la ficha está por debajo de la galería. -->
